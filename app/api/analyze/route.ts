...
function buildPrompt(
  ...
  const hotcookNote = has_hotcook
    ? `- 調理器具: ホットクックあり。以下の優先順位で料理を選ぶこと:
  優先1（自動メニューで対応可能）: 煮物（肉じゃが・筑前煮・角煮・魚の煮つけ）、無水カレー・シチュー・スープ・汁物・リゾット・炊き込みごはん・蒸し物・低温調理・ラタトゥイユ・ひじき・きんぴら
  優先2（手動モードで対応可能）: 炒め煮（もやし・豚肉・きのこ）、ポタージュ、ゆで野菜
  提案しない: 揚げ物、焦げ目が必要な焼き物（ステーキ・餃子）、シャキシャキ炒め（チャーハン・焼きそば・野菜炒め）
  ※ 優先1の料理が提案できる場合は必ず優先1を選ぶこと`
    : "";
